...
export const AGENTIC_STAGES: { key: AgenticStage; label: string; question: string }[] = [
  ...
  { key: "scheduling", label: "Initiation", question: "What starts the work when nobody asks?" },
...
export const AGENTIC: AgenticEntry[] = [
  ...
  },
  {
    ...
    whatItIs: "One row per agent or workspace recording what it writes, what it reads, what it may start, where it must stop, when it runs and what it may spend — with a constraint that refuses two agents claiming the same artefact.",
    ...
    watchFor: "A rule nobody can check is a preference — if the registry cannot fail, it is documentation. And a registry that records only ownership constrains collisions while leaving initiation with a person: record what each agent may START and where it must STOP, or the estate stays a catalogue.",
  ...
  /* ------------------------- Scheduling & autonomy ---------------------- */
  {
    ...
    whenYouNeedIt: "When real work is waiting on a person who is asleep — or on a person who is billing someone else for the afternoon.",
    notYet: "If nobody is blocked overnight, a schedule adds cost and removes the review step.",
  },
  {
    name: "Execution ledger", group: "Scheduling & autonomy", stage: "scheduling", standing: "Current", scale: "solo", door: "two-way", regime: "both",
    whatItIs: "The work tracker doubles as the queue: every sufficiently specified item carries an execution mode — autonomous, review, prepare, human, blocked — and agents read it to find what they are allowed to start.",
    whereYouMeetIt: "Plumbline's own Linear, after the 2026-09-02 recalibration. Any system where the backlog is already the source of truth about what needs doing.",
    watchFor: "This is the piece whose absence turns an agent estate into a catalogue. Without it the tools all exist and a person still has to start each one — which is the same bottleneck wearing a more sophisticated coat. The mode has to be durable state on the item, not a convention in someone's head.",
    needs: [], feeds: ["Scheduled deployments", "Worker cron", "Agent registry"], instead: [],
    whenYouNeedIt: "The moment you notice you are the thing that remembers, initiates and routes. That is a bottleneck, not a role.",
    notYet: "One agent doing one job on a fixed schedule does not need a queue — cron is the whole answer.",
  },
  {
    name: "Commitment gates", group: "Scheduling & autonomy", stage: "scheduling", standing: "Current", scale: "solo", door: "two-way", regime: "both",
    whatItIs: "Human review placed at the irreversible step rather than at the start. Preparatory and reversible work runs unasked; the person is asked only for the commitment, the judgement, or the relationship.",
    whereYouMeetIt: "The difference between a system that saves you time and one that adds an approval queue to your day.",
    watchFor: "Approval placed at initiation feels safe and is expensive: it makes a person the scheduler for work that was never risky. Put the gate where something becomes hard to undo — sending, publishing, paying, deleting, overwriting, committing on someone's behalf — and let everything before it run.",
    needs: ["Execution ledger"], feeds: ["Approval gates"], instead: [],
    whenYouNeedIt: "As soon as you are approving things you would never have refused.",
    notYet: "If every action the agent can take is genuinely irreversible, the gate is at the start because that is where the commitment is.",
  },
  {
